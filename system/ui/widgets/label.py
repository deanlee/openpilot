...
class TextLayout:
  def __init__(self, text: str = "", font: rl.Font = None,
               font_size: int = 16, max_width: int = 0,
               text_color: rl.Color = rl.BLACK,
               text_alignment: int = rl.GuiTextAlignment.TEXT_ALIGN_LEFT,
               text_alignment_vertical: int = rl.GuiTextAlignmentVertical.TEXT_ALIGN_TOP,
               icon: rl.Texture = None):
    self._text: str = text
    self._text_color: rl.Color = text_color
    self._width: int = 0
    self._font: rl.Font | None = font
    self._font_size: int = font_size
    self._max_width: int = 0
    self._letter_spacing: float = 0.0
    self._text_alignment: int = text_alignment
    self._text_alignment_vertical: int = text_alignment_vertical
    self._icon = icon
  ...
  def set_font(self, font: rl.Font):
    if font != self._font:
      self._font = font
      self._elements.clear()
  ...
  def update_layout(self):
    ...
    if not self._text and not self._icon:
      return

    content_width = self._rect.width - self._text_padding * 2

    if self._elide_right:
      text_size = measure_text_cached(self._font, self._text, self._font_size)
      content_width = self._rect.width - self._text_padding * 2
      if self._icon:
      ...
      if text_size.x > content_width:
        ...
        while left < right:
          mid = (left + right) // 2
          candidate = self._text[:mid] + ellipsis
          candidate_size = measure_text_cached(self._font, candidate, self._font_size)
          if candidate_size.x <= content_width:
            left = mid + 1
          else:
            right = mid
        display_text = self._text[: left - 1] + ellipsis if left > 0 else ellipsis
      else:
        display_text = self._text

      wrapped_lines = [display_text] if display_text else []
    else:
      wrapped_lines = wrap_text(self._font, self._text, self._font_size, int(content_width)) if self._text else []
...
class Label(Widget):
  def __init__(self,
    ...
    self._text = text
    self._text_layout = TextLayout(_resolve_value(text),
                                   font=gui_app.font(font_weight),
                                   font_size=font_size,
  ...
  def set_text(self, text):
    self._text = text
    self._text_layout.set_text(_resolve_value(text))

  def set_text_color(self, color):
    self._text_layout.set_text_color(color)

  def set_font_size(self, size):
    self._text_layout.set_font_size(size)

  def _render(self, rect: rl.Rectangle):
    self._text_layout.render(rect)
