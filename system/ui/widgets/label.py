...
def _elide_text(font: rl.Font, text: str, font_size: int, max_width: float, spacing: int = 0) -> str:
  ...
  while left < right:
    ...
    if measure_text_cached(font, candidate, font_size, spacing).x <= max_width:
      left = mid + 1
    else:
      right = mid
...
class Label(Widget):
  def __init__(
    ...
    self._spacing = letter_spacing * size  # in pixels
    if width > 0:
      self.get_content_height(width)
  ...
  def get_content_height(self, max_width: int) -> float:
    ...
    if self._layout_cache:
      return self._layout_cache.content_height
    return 0.0
