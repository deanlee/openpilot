...
RemoteEncoder::RemoteEncoder(const std::string &name) : name(name) {
  for (const auto &cam : cameras_logged) {
    for (const auto &info : cam.encoder_infos) {
      if (name == info.publish_name) {
        encoder_info = info;
        break;
      }
    }
  }
  assert(name == encoder_info.publish_name);
}

int handle_encoder_msg(LoggerdState *s, Message *msg_ptr, struct RemoteEncoder &re) {
  int bytes_count = 0;
