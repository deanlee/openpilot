...
static cl_program build_debayer_program(const CameraInfo *ci, const CameraBuf *b, const CameraState *s) {
  char args[4096];
  ...
  return CLContext::buildProgramFromFile(cl_file, args);
}

void CameraBuf::init(CameraState *s, VisionIpcServer * v, int frame_cnt, VisionStreamType rgb_type, VisionStreamType yuv_type, release_cb release_callback) {
  vipc_server = v;
  ...
  for (int i = 0; i < frame_buf_count; i++) {
    ...
    camera_bufs[i].init_cl(CLContext::deviceId(), CLContext::context());
  }
  ...
  if (ci->bayer) {
    cl_program prg_debayer = build_debayer_program(ci, this, s);
    krnl_debayer = CL_CHECK_ERR(clCreateKernel(prg_debayer, "debayer10", &err));
    CL_CHECK(clReleaseProgram(prg_debayer));
  }

  rgb2yuv = std::make_unique<Rgb2Yuv>(rgb_width, rgb_height, rgb_stride);

#ifdef __APPLE__
  q = CL_CHECK_ERR(clCreateCommandQueue(CLContext::context(), CLContext::deviceId(), 0, &err));
#else
  const cl_queue_properties props[] = {0};  //CL_QUEUE_PRIORITY_KHR, CL_QUEUE_PRIORITY_HIGH_KHR, 0};
  q = CL_CHECK_ERR(clCreateCommandQueueWithProperties(CLContext::context(), CLContext::deviceId(), props, &err));
#endif
}
