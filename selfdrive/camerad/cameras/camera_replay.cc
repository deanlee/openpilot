...
void camera_init(MultiCameraState *server, CameraState *s, int camera_id, unsigned int fps, VisionStreamType rgb_type, VisionStreamType yuv_type, const std::string &url) {
  ...
  s->fps = fps;
  s->buf.init(server, s, FRAME_BUF_COUNT, rgb_type, yuv_type);
}
