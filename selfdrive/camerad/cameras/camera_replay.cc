...
void camera_init(MultiCameraState *server, CameraState *s, int camera_id, unsigned int fps, const std::string &url) {
...
void cameras_init(MultiCameraState *s) {
  camera_init(s, &s->road_cam, CAMERA_ID_LGC920, 20, get_url(road_camera_route, "fcamera", 0));
  // camera_init(s, &s->driver_cam, CAMERA_ID_LGC615, 10, get_url(driver_camera_route, "dcamera", 0));
}
