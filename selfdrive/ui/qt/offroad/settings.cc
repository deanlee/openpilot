...
DevicePanel::DevicePanel(QWidget* parent) : QWidget(parent) {
  ...
  device_layout->addWidget(new LabelControl("Dongle ID", params.get("DongleId").c_str()));
  device_layout->addWidget(horizontal_line());

  device_layout->addWidget(new LabelControl("Serial", params.get("HardwareSerial").c_str()));
  ...
  offroad_btns.append(createResetCalibBtn());
...
ButtonControl *DevicePanel::createResetCalibBtn() {
  QString resetCalibDesc = "openpilot requires the device to be mounted within 4° left or right and within 5° up or down. openpilot is continuously calibrating, resetting is rarely required.";
  ButtonControl *btn = new ButtonControl("Reset Calibration", "RESET", resetCalibDesc, [=]() {
    if (ConfirmationDialog::confirm("Are you sure you want to reset calibration?", this)) {
      Params().remove("CalibrationParams");
    }
  }, "", this);
  connect(btn, &ButtonControl::showDescription, [=]() {
