...
QPixmap AnnotatedCameraWidget::drawEUSpeedLimit() {
  ...
  QPoint center(outer_radius, outer_radius);
  p.setPen(Qt::NoPen);
  ...
  p.setPen(blackColor());
  p.drawText(pixmap.rect(), Qt::AlignCenter, speedLimitStr);
  return pixmap;
}

QPixmap AnnotatedCameraWidget::drawCurrentSpeed() {
  QFont speed_font = getFont(176, "Bold");
  QFont unit_font = getFont(66, "Regular");
  QString speedStr = QString::number(std::nearbyint(speed));
  auto speed_rect = QFontMetrics(speed_font).boundingRect(speedStr);
  auto  unit_rect = QFontMetrics(unit_font).boundingRect(speedUnit);
  QRect r(0, 0, std::max(speed_rect.width(), unit_rect.width()), speed_rect.height() + unit_rect.height());
  ...
  p.setFont(unit_font);
  p.drawText(r, Qt::AlignHCenter | Qt::AlignBottom, speedUnit);
  return pixmap;
}

QPixmap AnnotatedCameraWidget::drawUSSpeedLimit(int rect_width) {
...
void AnnotatedCameraWidget::drawHud(QPainter &p) {
  ...
  QPixmap max_speed_px;
  if (!QPixmapCache::find(cache_key, &max_speed_px)) {
    max_speed_px = drawMaxSpeed(rect_width);
    QPixmapCache::insert(cache_key, max_speed_px);
  }
  // QRect s
  QRect set_speed_rect(QPoint(60, 45), max_speed_px.size());
  p.drawPixmap(set_speed_rect.topLeft(), max_speed_px);
  ...
  if (true) { //has_us_speed_limit) {
    ...
    if (!QPixmapCache::find(cache_key, &pixmap)) {
      pixmap = drawUSSpeedLimit(rect_width);
      QPixmapCache::insert(cache_key, pixmap);
    }
    p.drawPixmap(set_speed_rect.topLeft() + QPoint((set_speed_rect.width() - pixmap.width()) / 2, (set_speed_rect.height() - pixmap.height()) / 2), pixmap);
  }
  ...
  if (true) { //(has_eu_speed_limit) {
    ...
    if (!QPixmapCache::find(cache_key, &pixmap)) {
      pixmap = drawEUSpeedLimit();
      QPixmapCache::insert(cache_key, pixmap);
    }
    p.drawPixmap(set_speed_rect.left(), set_speed_rect.bottom() + 11, pixmap);
  }

  // current speed
  cache_key = QString("current_speed_%1_%2").arg(speedStr).arg(speedUnit);
  if (!QPixmapCache::find(cache_key, &pixmap)) {
    pixmap = drawCurrentSpeed();
    QPixmapCache::insert(cache_key, pixmap);
  }
