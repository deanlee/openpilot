...
void update_leads(UIState *s, const cereal::RadarState::Reader &radar_state, const cereal::XYZTData::Reader &line) {
  for (int i = 0; i < 2; ++i) {
    ...
    if (lead_data.getStatus()) {
      ...
      s->scene.calibration.calib_frame_to_full_frame(lead_data.getDRel(), -lead_data.getYRel(), z + 1.22, &s->scene.lead_vertices[i]);
    }
  }
}
...
void update_line_data(const UIState *s, const cereal::XYZTData::Reader &line,
  ...
  for (int i = 0; i <= max_idx; i++) {
    ...
    bool l = s->scene.calibration.calib_frame_to_full_frame(line_x[i], line_y[i] - y_off, line_z[i] + z_off, &left);
    bool r = s->scene.calibration.calib_frame_to_full_frame(line_x[i], line_y[i] + y_off, line_z[i] + z_off, &right);
    if (l && r) {
...
static void update_state(UIState *s) {
  ...
  if (sm.updated("liveCalibration")) {
    s->scene.calibration.update(sm["liveCalibration"].getLiveCalibration());
  }
