...
class ModelRenderer(Widget):
  ...
  def _map_line_to_polygon(self, line: np.ndarray, y_off: float, z_off: float, max_idx: int, allow_invert: bool = True) -> np.ndarray:
    ...
    for i in range(min(max_idx + 1, len(line_x))):
      # Highly negative x positions are drawn above the frame and cause flickering, clip to zy plane of camera
      if line_x[i] < 0:
        continue

      # Map left and right points to screen
      left_point = self._map_to_screen(line_x[i], line_y[i] - y_off, line_z[i] + z_off)
      right_point = self._map_to_screen(line_x[i], line_y[i] + y_off, line_z[i] + z_off)

      if left_point and right_point:
        # For wider lines the drawn polygon will "invert" when going over a hill and cause artifacts
        if not allow_invert and left_points and left_point[1] > left_points[-1][1]:
          continue

        left_points.append(left_point)
        right_points.insert(0, right_point)  # Insert at front (equivalent to push_front)

    if not left_points:
      return np.empty((0, 2), dtype=np.float32)

    # Combine left points + reversed right points to form the polygon
