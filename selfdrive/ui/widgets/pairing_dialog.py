...
class PairingDialog:
  ...
  def render(self, rect: rl.Rectangle) -> int:
    ...
    color = rl.Color(180, 180, 180, 150) if mouse.is_down_in_rect(close_rect) else rl.WHITE
    rl.draw_texture(close_icon, int(content_rect.x), int(y), color)

    if (mouse.is_clicked(close_rect)) or rl.is_key_pressed(rl.KeyboardKey.KEY_ESCAPE):
      return 1
