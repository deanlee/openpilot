...
class Controls:
  ...
  def update_events(self, CS):
    ...
    def add_fan_event():
      ...
      if peripheralState.pandaType != log.PandaState.PandaType.unknown:
        if peripheralState.fanSpeedRpm < 500 and self.sm['deviceState'].fanSpeedPercentDesired > 50:
    ...
    def add_calibration_events():
      # Check calibration status and add relevant events.
    ...
    def add_lane_change_events():
      ...
      elif meta.laneChangeState in (LaneChangeState.laneChangeStarting, LaneChangeState.laneChangeFinishing):
        self.events.add(EventName.laneChange)

    def add_panda_events():
    ...
    def add_gps_events():
      liveLocationKalman = self.sm['liveLocationKalman']
      if not (self.CP.notCar and self.joystick_mode):
        if not liveLocationKalman.posenetOK:
          self.events.add(EventName.posenetInvalid)
        if not liveLocationKalman.deviceStable:
          self.events.add(EventName.deviceFalling)
        if not liveLocationKalman.inputsOK:
          self.events.add(EventName.locationdTemporaryError)
        if not liveLocationKalman.valid and not TESTING_CLOSET and (not SIMULATION or REPLAY):
          self.events.add(EventName.paramsdTemporaryError)

        if not SIMULATION or REPLAY:
          # Not show in first 1 km to allow for driving out of garage. This event shows after 5 minutes
          if not liveLocationKalman.gpsOK and liveLocationKalman.inputsOK and (self.distance_traveled > 1500):
