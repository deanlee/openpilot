...
EncoderState::EncoderState(const std::string &filename, int width, int height, int fps, int bitrate, bool h265, bool downscale)
...
bool EncoderState::ProcessFrame(uint64_t ts, const uint8_t *y_ptr, const uint8_t *u_ptr, const uint8_t *v_ptr, int in_width, int in_height, VIPCBufExtra *extra) {
  int err;
  ...
  err = libyuv::I420ToNV12(y_ptr, width,
                   u_ptr, width/2,
                   v_ptr, width/2,
...
void EncoderState::Open(const std::string &path) {
  ...
  if (remuxing) {
    ...
    err = avio_open(&ofmt_ctx->pb, path.c_str(), AVIO_FLAG_WRITE);
    assert(err >= 0);
