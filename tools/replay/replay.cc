...
void Replay::seekTo(double seconds, bool relative) {
  ...
  interruptStream([=]() {
    bool segment_loaded = isSegmentLoaded(target_segment);
    notifyEvent(onSeeking, target_time);

    if (segment_loaded) {
      seeking_to_ = std::nullopt;
      notifyEvent(onSeekedTo, target_time);
    } else {
...
void Replay::handleSegmentMerge() {
  interruptStream([this]() {
    event_data_mutex_.lock();
    event_data_ = seg_mgr_.getEventData();
    event_data_mutex_.unlock();

    bool segment_loaded = isSegmentLoaded(current_segment_);
    if (segment_loaded && seeking_to_) {
...
void Replay::streamThread() {
  ...
  while (true) {
    ...
    if (it != events->cend()) {
      ...
      if (current_segment_ >= last_segment && event_data_.segments.count(last_segment)) {
