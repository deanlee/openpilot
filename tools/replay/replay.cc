...
void Replay::publishFrame(SegmentManager::Events *segment_events, const Event *e) {
  CameraType cam;
  switch (e->which) {
  ...
  auto seg_it = segment_events->segments.find(e->eidx_segnum);
  if ( seg_it != segment_events->segments.end()) {
...
EventIterator Replay::publishEvents(SegmentManager::Events *segment_events, EventIterator first, EventIterator last) {
  ...
  for (; !paused_ && first != last; ++first) {
    ...
    if (evt.eidx_segnum == -1) {
      ...
      publishFrame(segment_events, &evt);
    }
  }
