...
SegmentManager::~SegmentManager() {
  {
    ...
    exit_ = true;
    onSegmentLoadedCallback_ = nullptr;
  }
  cv_.notify_one();
  if (thread_.joinable()) {
    thread_.join();
...
void SegmentManager::mergeSegments(const SegmentMap::iterator &begin, const SegmentMap::iterator &end) {
  ...
  for (int n : segments_to_merge) {
    ...
    merged_events.insert(merged_events.end(), segment_events.begin(), segment_events.end());
  ...
  if (onSegmentLoadedCallback_) {
    onSegmentLoadedCallback_();  // Notify listener that segments have been merged
  }
}
