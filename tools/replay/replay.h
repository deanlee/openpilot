...
public:
  ...
  bool isPaused() const { return user_paused_; }
  ...
  inline bool isSegmentLoaded(int n) const { return event_data_ && event_data_->segments.count(n); }
  const std::shared_ptr<std::vector<Timeline::Entry>> getTimeline() const { return timeline_.get(); }
  const std::optional<Timeline::Entry> findAlertAtTime(double sec) const { return timeline_.findAlertAtTime(sec); }
...
private:
  ...
  void publishFrame(SegmentManager::EventData *event_data, const Event *e);
  void checkSeekProgress();
  EventIterator publishEvents(SegmentManager::EventData *event_data, EventIterator first, EventIterator last);
  ...
  std::shared_ptr<SegmentManager::EventData> event_data_;
};
