...
private:
  ...
  void publishFrame(SegmentManager::Events *segment_events, const Event *e);
  void checkSeekProgress();
  void updateEvents(const std::function<bool()> &update_events_function);
  EventIterator publishEvents(SegmentManager::Events *segment_events, EventIterator first, EventIterator last);
