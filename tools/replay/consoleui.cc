...
void ConsoleUI::updateStatus() {
  ...
  auto cur_datetime = replay->routeDateTime().addSecs(replay->currentSeconds());
  std::string current_segment = " - " + std::to_string((int)(replay->currentSeconds() / 60));
  write_item(0, 25, "TIME:  ", cur_datetime.toString("ddd MMMM dd hh:mm:ss").toStdString(), current_segment, true);
