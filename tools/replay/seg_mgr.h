...
public:
  ...
  ~SegmentManager();
  bool load();
  void updateCurrentSegment(int seg_num);
  void setCallback(std::function<void()> callback) { onSegmentLoaded_ = callback; }
  void setFilters(const std::vector<bool> &filters) { filters_ = filters; };
  void setFlags(uint32_t flags) { flags_ = flags; }
  inline bool hasSegment(int n) const { return segments_.find(n) != segments_.end(); }
  inline const SegmentMap &getSegments() const { return segments_; }
  inline const std::shared_ptr<Events> events() const { return events_; }
...
private:
  ...
  std::function<void()> onSegmentLoaded_ = nullptr;
