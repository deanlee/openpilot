...
size_t AbstractStream::suppressHighlighted() {
  std::lock_guard lk(mutex_);
  size_t cnt = 0;
  const uint64_t cur_mono_time = currentMonoTime();
  for (auto &[_, m] : msgs_) {
    for (auto &last_change : m.last_changes) {
      const double dt = (cur_mono_time - last_change.mono_time) / 1e9;
      if (dt < 2.0) {
...
void AbstractStream::updateEvent(const MessageId &id, uint64_t current_ts, const uint8_t *data, uint8_t size) {
  std::lock_guard lk(mutex_);
  auto mask_it = masks_.find(id);
  std::vector<uint8_t> *mask = mask_it == masks_.end() ? nullptr : &mask_it->second;
  msgs_[id].compute(id, data, size, current_ts, getSpeed(), mask);
  new_msgs_.insert(id);
}
...
double calc_freq(const MessageId &msg_id, uint64_t current_ts) {
  ...
  uint64_t first_ts = current_ts - std::min<uint64_t>(current_ts, 59 * 1e9);
...
void CanData::compute(const MessageId &msg_id, const uint8_t *can_data, const int size, uint64_t current_ts,
  ...
  if (current_ts < last_freq_update_ts || (current_ts - last_freq_update_ts) >= 1e9) {
