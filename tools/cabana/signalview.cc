...
QModelIndex SignalView::signalIndexAt(const QPoint &pt) {
  QModelIndex index = tree->indexAt(pt);
...
void SignalView::updateState(const std::set<MessageId> *msgs) {
  ...
  QModelIndex first_visible_index = signalIndexAt(QPoint(0, 0));
  if (first_visible_index.isValid()) {
    auto last_visible_index = signalIndexAt(tree->viewport()->rect().bottomLeft());
    if (!last_visible_index.isValid()) {
      last_visible_index = model->index(model->rowCount() - 1, 0);
    }
    ...
    for (int i = first_visible_index.row(); i <= last_visible_index.row(); ++i) {
